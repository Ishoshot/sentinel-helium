    ...
    <table class="min-w-full divide-y divide-border-subtle">
      <thead class="bg-bg-surface">
        <tr>
          <th
            scope="col"
            class="px-4 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider w-12"
          >
            Status
          </th>
          <th
          ...
          <th
            scope="col"
            class="px-4 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider w-48"
          >
            Repository
          </th>
          <th
            scope="col"
            class="px-4 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider w-32"
          >
            Author
          </th>
          <th
            scope="col"
            class="px-4 py-3 text-right text-xs font-medium text-text-secondary uppercase tracking-wider w-24"
          >
            Duration
          </th>
          <th
            scope="col"
            class="px-4 py-3 text-right text-xs font-medium text-text-secondary uppercase tracking-wider w-32"
          >
            Date
          </th>
        </tr>
      ...
      <tbody class="bg-bg-elevated divide-y divide-border-subtle">
        <tr 
          v-for="run in runs" 
          :key="run.id"
          class="hover:bg-bg-surface/50 transition-colors group cursor-pointer"
        ...
        >
          ...
          <td class="px-4 py-3">
            <div class="flex flex-col gap-0.5">
              <span class="text-sm font-medium text-text-primary truncate max-w-md">
                {{ run.pull_request?.title ?? run.metadata?.pull_request_title ?? `Run #${run.id}` }}
              </span>
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-1.5 text-sm text-text-secondary">
              ...
              />
              <span class="truncate max-w-[180px]">{{ getRepositoryName(run) }}</span>
            </div>
          </td>
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-2">
              <BaseAvatar
                ...
                size="sm"
              />
              <span class="text-sm text-text-secondary truncate max-w-[100px]">
                {{ getAuthor(run)?.login }}
              </span>
            </div>
          </td>
          ...
          <!-- Duration -->
          <td class="px-4 py-3 whitespace-nowrap text-right text-sm text-text-secondary tabular-nums">
            {{ formatDuration(run.metrics?.duration_ms) }}
          </td>

          <!-- Date -->
          <td class="px-4 py-3 whitespace-nowrap text-right text-sm text-text-secondary">
