...
**Action Required:** Remove all frontend support for `review_rules` (types, UI, and PATCH payloads).
...
4. **Keep `auto_review_enabled`** - This toggle remains (it's a repository-level kill switch)

### Current Frontend Touch Points (Must Update)

- `app/types/index.ts`
  - `RepositorySettings.review_rules`
  - `UpdateRepositoryData.review_rules`
- `app/components/domain/RepositorySettingsModal.vue`
  - “Review Rules (Advanced)” UI
  - Sends `review_rules` in `handleSave()`

When removing `review_rules`, ensure `PATCH /api/workspaces/{id}/repositories/{id}` only receives:
...
### Repository Settings (Current UI: Modal)

There is no dedicated repository settings page today.

Repository settings are edited via `DomainRepositorySettingsModal`, opened from:

- `app/pages/[workspace]/repositories/index.vue`

Prefer extending the existing modal unless product explicitly requires a standalone page.

Add a new "Sentinel Configuration" section to the modal.
...
## Types & Data Flow (Current Frontend)

Repository data currently flows through:

- `app/services/githubService.ts` (API calls)
- `app/composables/useGitHub.ts` (state + mutations)
- `app/pages/[workspace]/repositories/index.vue` (list/grid + settings modal)
- `app/types/index.ts` (API response types)

Update the existing `Repository` / `RepositorySettings` interfaces to include Sentinel config fields.

```typescript
// app/types/index.ts (updated)
...
export const useSentinelConfig = (settings: RepositorySettings | null) => {
  ...
  const configSummary = computed(() => {
    ...
    return {
      ...
      enabledCategories: Object.entries(config.value.review?.categories ?? {})
...
5. **Config display format** - The API returns `sentinel_config` as a parsed object. Prefer rendering it as pretty-printed JSON via `JSON.stringify(config, null, 2)` until we intentionally add a YAML serializer dependency.

6. **Documentation link** - Link to documentation about how to create and configure `.sentinel/config.yaml`. The exact doc location/URL is TBD.
...
// Note: repository state is managed in `app/composables/useGitHub.ts`.
// The list repositories payload should include the new settings fields.
// Update mutation should only send `{ auto_review_enabled }`.
```
...
- [ ] Remove `review_rules` support from frontend types + UI
- [ ] Update `RepositorySettings` TypeScript interface with new config fields
- [ ] Update `UpdateRepositoryData` to only allow `auto_review_enabled`
- [ ] Create `useSentinelConfig` composable for config display state
- [ ] Add config status indicator to repository list items (`DomainRepositoryRow` / `DomainRepositoryCard`)
- [ ] Add "Sentinel Configuration" section in `DomainRepositorySettingsModal`
- [ ] Handle all three states: default, active, error
- [ ] Create full config viewer modal (read-only)
- [ ] Render config as pretty-printed JSON (unless we add a YAML serializer)
...
- [ ] Make config error state visually prominent (warning styling, iconography)
